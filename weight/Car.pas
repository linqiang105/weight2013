...
uses
  Classes, Controls, Forms, Types, Buttons, StdCtrls, ExtCtrls, ComCtrls;
...
uses Main, RzCmboBx, Dialogs, IniFiles, SysUtils, PrepareUtil, QueryDM,
  ManualInput;
...
begin
  CBCar.Text := CBCar.Text + (Sender as TButton).Caption;
  CBCar.SetFocus;
  CBCar.SelStart := Length(CBCar.Text);
end;
...
begin
  CBCar.Text := Copy(CBCar.Text, 1, Length(CBCar.Text) - 1);
end;
...
begin
  try
    MainForm.FrmWeight1.CBCar.SetFocus;
  except
  end;
end;
...
begin
  ...
  try
    ...
    BtnNum0.Caption := myini.ReadString('key', 'num0', '0');
...
procedure TCarForm.FormDestroy(Sender: TObject);
var myini: TIniFile;
begin
  myini := TIniFile.Create(ExtractFilePath(ParamStr(0)) + 'car.ini');
  try
