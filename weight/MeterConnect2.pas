...
type
  TMeterConnect2Form = class(TForm)
    PTitle: TPanel;
    TSend: TTimer;
    IdUDPC: TIdUDPClient;
    IdUDPS: TIdUDPServer;
...
var
  MeterConnect2Form: TMeterConnect2Form;
...
procedure TMeterConnect2Form.Commpopen;
begin
  ...
  try
    ...
    begin
      MessageDlg('打开2#仪表串口失败！' + Char(13) + '请检查串口连接是否设置正确！',
        mtWarning, [mbOK], 0);
      Exit;
    end;
...
procedure TMeterConnect2Form.BitBtnCancelClick(Sender: TObject);
begin
  Close
end;

procedure TMeterConnect2Form.BitBtnApplyClick(Sender: TObject);
var myini: Tinifile;
begin
  myini := Tinifile.Create(ExtractFilePath(ParamStr(0)) + '\meter.ini');
  try
    myini.WriteBool('meter2', 'use', CBUseMeter.Checked);
    myini.Writeinteger('meter2', 'meter_type', CBMeterType.ItemIndex);
    myini.Writeinteger('meter2', 'com', ComPort.ItemIndex);
    myini.Writeinteger('meter2', 'baut', BaudRate.ItemIndex);
    myini.Writeinteger('meter2', 'databit', ByteSize.ItemIndex);
    myini.Writeinteger('meter2', 'stopbit', StopBits.ItemIndex);
    myini.Writeinteger('meter2', 'parity', Parity.ItemIndex);
    myini.Writeinteger('meter2', 'polling', PollingDelay.Value);

    myini.WriteInteger('meter2', 'type', CBType.ItemIndex);
    myini.WriteString('meter2', 'start', LEStart.Text);
    myini.WriteString('meter2', 'first', LEGetFirst.Text);
    myini.WriteString('meter2', 'len', LEDataLength.Text);
    myini.Writeinteger('meter2', 'queue', CBQueue.ItemIndex);
    myini.WriteString('meter2', 'point', LEPoint.Text);

    myini.WriteBool('meter2', 'to_screen', CBScreen.Checked);
    myini.WriteBool('meter2', 'to_screen_type', RBAlways.Checked);

    myini.Writeinteger('meter2', 'meter_unit_index', CBMeterUnit.ItemIndex);
    myini.WriteString('meter2', 'meter_unit', CBMeterUnit.Text);
    myini.Writeinteger('meter2', 'soft_unit_index', CBSoftUnit.ItemIndex);
    myini.WriteString('meter2', 'soft_unit', CBSoftUnit.Text);

    MainForm.systemConfig.softUnit2 := CBSoftUnit.Text;
    MainForm.FrmWeight1.lblUnit2.Caption := CBSoftUnit.Text;

    myini.WriteString('meter2', 'meter_show_mode', LEMeterShowMode.Text);
    MainForm.systemConfig.meterShowMode2 := LEMeterShowMode.Text;
    myini.WriteInteger('meter2', 'meter_divide', StrToIntDef(CBMeterDivide.Text, 10));
    MainForm.systemConfig.meterDivide2 := StrToIntDef(CBMeterDivide.Text, 10);

    reloadUnit;

    myini.WriteBool('meter2', 'allowRemote', CBAllowRemote.Checked);
    myini.WriteBool('meter2', 'readRemote', CBUDP.Checked);
    myini.WriteString('meter2', 'remoteIp', LEIP.Text);
    myini.WriteInteger('meter2', 'remotePort', StrToIntDef(LEPort.Text, 8888));

    myini.WriteString('meter2', 'send_str', EdtSend.Text);
    myini.WriteBool('meter2', 'auto_send', CBAutoSend.Checked);

    myini.WriteInteger('meter2', 'screen_type', CBScreenType.ItemIndex);
...
procedure TMeterConnect2Form.PollingDelayKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TMeterConnect2Form.CBMeterTypeChange(Sender: TObject);
begin
  Commpopen;
end;

procedure TMeterConnect2Form.FormCreate(Sender: TObject);
var myini: Tinifile;
  i: Integer;
begin
  for i := 1 to 16 do
  ...
  myini := Tinifile.Create(ExtractFilePath(ParamStr(0)) + '\meter.ini');
  try
    CBUseMeter.Checked := myini.ReadBool('meter2', 'use', False);
    CBMeterType.ItemIndex := myini.ReadInteger('meter2', 'meter_type', 1);
    ComPort.ItemIndex := myini.ReadInteger('meter2', 'com', 0);
    BaudRate.ItemIndex := myini.ReadInteger('meter2', 'baut', 5);
    ByteSize.ItemIndex := myini.ReadInteger('meter2', 'databit', 3);
    StopBits.ItemIndex := myini.ReadInteger('meter2', 'stopbit', 0);
    Parity.ItemIndex := myini.ReadInteger('meter2', 'parity', 0);
    PollingDelay.Value := myini.ReadInteger('meter2', 'polling', 300);

    CBType.ItemIndex := myini.ReadInteger('meter2', 'type', 0);
    LEStart.Text := myini.ReadString('meter2', 'start', '=');
    LEGetFirst.Text := myini.ReadString('meter2', 'first', '1');
    LEDataLength.Text := myini.ReadString('meter2', 'len', '7');
    CBQueue.ItemIndex := myini.ReadInteger('meter2', 'queue', 0);
    LEPoint.Text := myini.ReadString('meter2', 'point', '0');

    CBScreen.Checked := myini.ReadBool('meter2', 'to_screen', False);
    RBAlways.Checked := myini.ReadBool('meter2', 'to_screen_type', True);
    RBAfterSave.Checked := not myini.ReadBool('meter2', 'to_screen_type', True);

    CBMeterUnit.ItemIndex := myini.ReadInteger('meter2', 'meter_unit_index', 0);
    CBSoftUnit.ItemIndex := myini.ReadInteger('meter2', 'soft_unit_index', 0);
    LEMeterShowMode.Text := myini.ReadString('meter2', 'meter_show_mode', '0');
    CBMeterDivide.Text := myini.ReadString('meter2', 'meter_divide', '1');

    reloadUnit;

    CBUDP.Checked := myini.ReadBool('meter2', 'readRemote', False);
    LEIP.Text := myini.ReadString('meter2', 'remoteIp', '127.0.0.1');
    LEPort.Text := myini.ReadString('meter2', 'remotePort', '8888');
    CBAllowRemote.Checked := myini.ReadBool('meter2', 'allowRemote', False);

    EdtSend.Text := myini.ReadString('meter2', 'send_str', '');
    CBAutoSend.Checked := myini.ReadBool('meter2', 'auto_send', False);

    CBScreenType.ItemIndex := myini.ReadInteger('meter2', 'screen_type', 0);
...
procedure TMeterConnect2Form.CommPReceiveData(Sender: TObject;
...
begin
  ...
  if s <> '' then
    MainForm.FrmWeight1.AniTrans2.ImageIndex := 1;
  ...
  try
    ...
    MainForm.FrmWeight1.PWeight2.Caption := FormatFloat(MainForm.systemConfig.meterShowMode1,
...
procedure TMeterConnect2Form.SBClearClick(Sender: TObject);
begin
  MemoLog.Lines.Clear;
end;

procedure TMeterConnect2Form.SBSendClick(Sender: TObject);
begin
  if CommP.Connected then
  begin
...
procedure TMeterConnect2Form.reloadUnit;
...
procedure TMeterConnect2Form.TSendTimer(Sender: TObject);
var s: string;
begin
...
procedure TMeterConnect2Form.CBScreenClick(Sender: TObject);
begin
  TScreen.Enabled := CBScreen.Checked;
end;

procedure TMeterConnect2Form.TScreenTimer(Sender: TObject);
  ...
  try
    ...
    begin
      if MainForm.FrmWeight1.saved then
        WeightTemp := MainForm.FrmWeight1.PWeight2.Caption
      else
        WeightTemp := '0';
    end;
    OutWeight := '';
...
procedure TMeterConnect2Form.TUDPClientTimer(Sender: TObject);
var s: string;
  i: integer;
begin
      ...
      try
        ...
        begin
          ...
          begin
            if s[i] = Chr($02) then
            begin
              MainForm.FrmWeight1.PWeight2.Caption := FloatToStr(StrToFloatDef(Copy(s, i + 1, 8), 0));
              break;
            end;
          end;
        end;
...
procedure TMeterConnect2Form.IdUDPSUDPRead(Sender: TObject; AData: TStream;
  ABinding: TIdSocketHandle);
var
...
begin
  ...
  try
    ...
    begin
      r := MainForm.FrmWeight1.PWeight2.Caption;
...
procedure TMeterConnect2Form.CBAllowRemoteClick(Sender: TObject);
begin
  //IdUDPS.Binding.IP := '0.0.0.0';
  IdUDPS.DefaultPort := StrToIntDef(LEPort.Text, 8888);
  IdUDPS.Active := CBAllowRemote.Checked;
...
procedure TMeterConnect2Form.CBUDPClick(Sender: TObject);
begin
  IdUDPC.Host := LEIP.Text;
  IdUDPC.Port := StrToIntDef(LEPort.Text, 8888);
...
procedure TMeterConnect2Form.addMemo(s: string);
begin
...
procedure TMeterConnect2Form.setDefineState(meterIndex: Integer);
var i: Integer;
begin
...
procedure TMeterConnect2Form.setSenderState(meterIndex: Integer);
begin
  TSend.Interval := PollingDelay.Value;
  TSend.Enabled := True;
end;

procedure TMeterConnect2Form.CBSoftUnitChange(Sender: TObject);
begin
...
procedure TMeterConnect2Form.TTestTimer(Sender: TObject);
var i: Integer;
...
begin
  ...
  if comDisconnect then
    MainForm.FrmWeight1.PWeight2.Caption := '0';
end;

procedure TMeterConnect2Form.FormShow(Sender: TObject);
begin
