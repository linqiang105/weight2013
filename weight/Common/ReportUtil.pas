...
type
  ...
  public
    ...
    class function printDefaultTicket(glideNo: string; preview: Boolean = False; ticketType: Integer = 1): Boolean;
    ...
    //扫描打印报表
    class function showScanRecord(strGlideNos: TStrings): Boolean;
  end;

implementation

uses QueryDM, Main, TicketSel, CommonUtil, IdGlobal, Windows, DataQuery, Consts,
  Controls;
...
begin
  ...
  begin
    ...
    begin
      if wt = 'PO' then
        format := ExtractFilePath(ParamStr(0)) + MainForm.systemConfig.poTicketFormat
      else if wt = 'SO' then
        format := ExtractFilePath(ParamStr(0)) + MainForm.systemConfig.soTicketFormat
      else if wt = 'IO' then
        format := ExtractFilePath(ParamStr(0)) + MainForm.systemConfig.ioTicketFormat
      else if wt = 'OO' then
        format := ExtractFilePath(ParamStr(0)) + MainForm.systemConfig.ooTicketFormat
      else
        format := ExtractFilePath(ParamStr(0)) + MainForm.systemConfig.poTicketFormat
    end
    else if MainForm.systemConfig.ticketType = 2 then
    begin
      if wt = 'PO' then
        format := ExtractFilePath(ParamStr(0)) + MainForm.systemConfig.poTicketFormat2
      else if wt = 'SO' then
        format := ExtractFilePath(ParamStr(0)) + MainForm.systemConfig.soTicketFormat2
      else if wt = 'IO' then
        format := ExtractFilePath(ParamStr(0)) + MainForm.systemConfig.ioTicketFormat2
      else if wt = 'OO' then
        format := ExtractFilePath(ParamStr(0)) + MainForm.systemConfig.ooTicketFormat2
      else
        format := ExtractFilePath(ParamStr(0)) + MainForm.systemConfig.poTicketFormat2
    end;
  end;
  ...
  begin
    ...
    //先退纸
    TCommonUtil.printerBack(MainForm.systemConfig.printer1, MainForm.systemConfig.printerBackRow);
    if preview then
    begin
    ...
    begin
      ...
      QueryDataModule.frxReport1.PrintOptions.ShowDialog := MainForm.systemConfig.showPrintDialog;
  ...
  begin
    ...
    begin
      ...
      QueryDataModule.RMGridReport1.ShowPrintDialog := MainForm.systemConfig.showPrintDialog;
...
class function TReportUtil.printDefaultTicket(glideNo: string;
  preview: Boolean; ticketType: Integer): Boolean;
var format: string;
  hasNet: Boolean;
  wt: string;
begin
  ...
  begin
    ...
    begin
      wt := FieldByName('过磅类型').AsString;
      hasNet := FieldByName('净重').AsFloat > 0;
      if MainForm.systemConfig.printNet then
      begin
        if not hasNet then Exit;
      end;
    end;
  end;
  //磅单格式
  ...
  begin
    ...
    try
      TicketSelForm.ShowModal;
      if TicketSelForm.ModalResult = mrOK then
      begin
        if MainForm.systemConfig.ticketType = 1 then
          format := ExtractFilePath(ParamStr(0)) + '\Ticket\'
            + TicketSelForm.LBTicket.Items[TicketSelForm.LBTicket.ItemIndex] + '.fr3'
...
class function TReportUtil.showScanRecord(strGlideNos: TStrings): Boolean;
var i: Integer;
  sqlstr: string;
begin
  ...
  begin
    Close;
    if strGlideNos.Count = 0 then
    begin
      sqlstr := 'select * from 称重信息 where (0=1)';
      SQL.Text := sqlstr;
    end
    else
    begin
      sqlstr := 'select * from 称重信息 where (流水号 in (';
      for i := 0 to strGlideNos.Count - 1 do
      begin
        sqlstr := sqlstr + Format(':p%d', [i]) + ',';
      end;
      ...
      for i := 0 to strGlideNos.Count - 1 do
      begin
        Parameters.ParamByName(Format('p%d', [i])).Value := strGlideNos[i];
      end;
    end;
    Open;
  end;
...
class function TReportUtil.printTicket(glideNo: string; preview: Boolean;
  ticketType: Integer): Boolean;
begin
  if MainForm.systemConfig.defaultPrintPicTicket then
  begin
    printPicTicket(glideNo, preview, ticketType)
  end
  else
  begin
    printDefaultTicket(glideNo, preview, ticketType);
    //在两台打印机上打印
    if MainForm.systemConfig.printTwo then
      printPicTicket(glideNo, preview, ticketType);
  end;
end;
