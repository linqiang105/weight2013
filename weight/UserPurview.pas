...
uses
  ...
  DBGridEhToolCtrls, DBAxisGridsEh, DBGridEhGrouping, Buttons;
...
type
  TUserPurviewForm = class(TForm)
    ...
    procedure DBGridEhUserDblClick(Sender: TObject);
  private
...
uses QueryDM, Main, CommonUtil, OtherUtil;
{$R *.dfm}
...
procedure TUserPurviewForm.DBGridEhUserDblClick(Sender: TObject);
var u: string;
begin
  u := DBGridEhUser.DataSource.DataSet.FieldByName('用户名').AsString;
  if u <> '' then
  begin
    if Application.MessageBox('这将重置该用户密码，是否继续？', '提示',
      MB_YESNO + MB_ICONWARNING + MB_DEFBUTTON2 + MB_TOPMOST) = IDYES then
    begin
      TUserUtil.resetPass(u);
      Application.MessageBox('用户密码已重置！', '提示', MB_OK +
