...
begin
  ...
  try
    ...
    begin
      if not ForceDirectories(myini.ReadString('db', 'backup_addr', 'D:\ATWBACKUP\')) then
      begin
        MessageDlg('数据库备份目录创建失败，无法进行备份！', mtWarning, [mbOK], 0);
        Self.Close;
    ...
    lblDbAddr.Caption := myini.ReadString('db', 'backup_addr', 'D:\ATWBACKUP\')
      + FormatDateTime('yyyy-MM-dd hh-mm-ss', Now) + '.mdb';
