...
begin
  ...
  begin
    ...
    quanter := DataSet['方量'];
      ...
      with MainForm do
      begin
        if weightSetConfig.useExpression6 then
        begin
          DataSet['备用6'] := TExpressionUtil.getExpressionValue(weightSetConfig.expression6,
            FloatToStr(gross), FloatToStr(tare), FloatToStr(net), FloatToStr(buckle), FloatToStr(actual),
            FloatToStr(price), FloatToStr(sum), FloatToStr(scale), FloatToStr(quanter), '', '', '',
            '', '', weightSetConfig.point6);
        end;
        if weightSetConfig.useExpression7 then
        begin
          DataSet['备用7'] := TExpressionUtil.getExpressionValue(weightSetConfig.expression7,
            FloatToStr(gross), FloatToStr(tare), FloatToStr(net), FloatToStr(buckle), FloatToStr(actual),
            FloatToStr(price), FloatToStr(sum), '', '', '',
            '', '', DataSet['备用6'], weightSetConfig.point7);
        end;
        if weightSetConfig.useExpression8 then
        begin
          DataSet['备用8'] := TExpressionUtil.getExpressionValue(weightSetConfig.expression8,
            FloatToStr(gross), FloatToStr(tare), FloatToStr(net), FloatToStr(buckle), FloatToStr(actual),
            FloatToStr(price), FloatToStr(sum), '', '', '',
            '', '', DataSet['备用6'], DataSet['备用7'], weightSetConfig.point8);
        end;
        if weightSetConfig.useExpression9 then
        begin
          DataSet['备用9'] := TExpressionUtil.getExpressionValue(weightSetConfig.expression9,
            FloatToStr(gross), FloatToStr(tare), FloatToStr(net), FloatToStr(buckle), FloatToStr(actual),
            FloatToStr(price), FloatToStr(sum), '', '', '',
            '', '', DataSet['备用6'], DataSet['备用7'], DataSet['备用8'], weightSetConfig.point9);
        end;
      end;}
  end
end;
