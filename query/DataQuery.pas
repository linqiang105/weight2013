...
uses
  ...
  RM_FormReport, RM_PEHGrid, Grids, DBGridEhGrouping, RM_Class;
...
type
  ...
  TDataQueryForm = class(TForm)
    ...
    procedure NStat1Click(Sender: TObject);
    ...
    procedure NStat2Click(Sender: TObject);
    procedure NStat3Click(Sender: TObject);
    procedure NStat4Click(Sender: TObject);
    procedure NStat5Click(Sender: TObject);
    procedure NStat6Click(Sender: TObject);
    procedure NStat7Click(Sender: TObject);
...
begin
  ...
  begin
    ...
    begin
      try
        if DBGridEhWeight.SelectedIndex <> -1 then
          i := DBGridEhWeight.DataSource.DataSet.FieldByName('流水号').AsString;
        DBGridEhWeight.DataSource.DataSet.Delete;
...
var SelNo: string;
begin
  SelNo := DBGridEhWeight.DataSource.DataSet.FieldByName('流水号').AsString;
  if SelNo <> '' then
  begin
    TWeightUtil.showPhotoEx(selno, MainForm.systemConfig.savePicLocal);
    with DBGridEhWeight.DataSource.DataSet do
    begin
...
begin
  ...
  for i := 0 to DBGridEhStat.Columns.count - 1 do
  begin
    if (DBGridEhStat.Columns[i].FieldName = MainForm.weightSetConfig.grossCaption)
      or (DBGridEhStat.Columns[i].FieldName = MainForm.weightSetConfig.tareCaption)
      or (DBGridEhStat.Columns[i].FieldName = MainForm.weightSetConfig.netCaption)
      or (DBGridEhStat.Columns[i].FieldName = MainForm.weightSetConfig.bundleCaption)
      or (DBGridEhStat.Columns[i].FieldName = MainForm.weightSetConfig.realCaption)
      or (DBGridEhStat.Columns[i].FieldName = MainForm.weightSetConfig.sumCaption)
      or (DBGridEhStat.Columns[i].FieldName = MainForm.weightSetConfig.costCaption)
      or (DBGridEhStat.Columns[i].FieldName = '车次')
      or (DBGridEhStat.Columns[i].FieldName = MainForm.weightSetConfig.backup6Caption)
      or (DBGridEhStat.Columns[i].FieldName = MainForm.weightSetConfig.backup7Caption)
      or (DBGridEhStat.Columns[i].FieldName = MainForm.weightSetConfig.backup8Caption)
      or (DBGridEhStat.Columns[i].FieldName = MainForm.weightSetConfig.backup9Caption)
      or (DBGridEhStat.Columns[i].FieldName = MainForm.weightSetConfig.quanterCaption) then
...
procedure TDataQueryForm.NStat1Click(Sender: TObject);
begin
  if not TUserUtil.HasAuthority('打印报表') then
    Exit;

  TReportUtil.printStatReport(ExtractFilePath(ParamStr(0)) + '\StatReport\统计报表1.fr3');
end;
...
begin
  ...
  if CBGross.Checked then
    sqlstr := sqlstr + 'Sum(毛重) As [' + MainForm.weightSetConfig.grossCaption + '],';
  if CBTare.Checked then
    sqlstr := sqlstr + 'Sum(皮重) As [' + MainForm.weightSetConfig.tareCaption + '],';
  if CBNet.Checked then
    sqlstr := sqlstr + 'Sum(净重) As [' + MainForm.weightSetConfig.netCaption + '],';
  if CBBundle.Checked then
    sqlstr := sqlstr + 'Sum(扣重) As [' + MainForm.weightSetConfig.bundleCaption + '],';
  if CBReal.Checked then
    sqlstr := sqlstr + 'Sum(实重) As [' + MainForm.weightSetConfig.realCaption + '],';
  if CBSum.Checked then
    sqlstr := sqlstr + 'Sum(金额) As [' + MainForm.weightSetConfig.sumCaption + '],';
  if CBQuanter.Checked then
    sqlstr := sqlstr + 'Sum(方量) As [' + MainForm.weightSetConfig.quanterCaption + '],';
  if CBCost.Checked then
    sqlstr := sqlstr + 'Sum(过磅费) As [' + MainForm.weightSetConfig.costCaption + '],';
  if CBBackup6.Checked then
    sqlstr := sqlstr + 'Sum(备用6) As [' + MainForm.weightSetConfig.backup6Caption + '],';
  if CBBackup7.Checked then
    sqlstr := sqlstr + 'Sum(备用7) As [' + MainForm.weightSetConfig.backup7Caption + '],';
  if CBBackup8.Checked then
    sqlstr := sqlstr + 'Sum(备用8) As [' + MainForm.weightSetConfig.backup8Caption + '],';
  if CBBackup9.Checked then
    sqlstr := sqlstr + 'Sum(备用9) As [' + MainForm.weightSetConfig.backup9Caption + '],';
  if CBBackup15.Checked then
    sqlstr := sqlstr + 'Sum(备用15) As [' + MainForm.weightSetConfig.backup15Caption + '],';
  if CBBackup16.Checked then
    sqlstr := sqlstr + 'Sum(备用16) As [' + MainForm.weightSetConfig.backup16Caption + '],';
  if CBBackup17.Checked then
    sqlstr := sqlstr + 'Sum(备用17) As [' + MainForm.weightSetConfig.backup17Caption + '],';
  if CBBackup18.Checked then
    sqlstr := sqlstr + 'Sum(备用18) As [' + MainForm.weightSetConfig.backup18Caption + '],';
...
begin
  if not TUserUtil.HasAuthority('修改磅单') then
    Exit;
  TReportUtil.designReport(ExtractFilePath(ParamStr(0)) + '\StatReport\统计报表1.fr3');
end;
...
begin
  ...
  statShowName.Add('');
  statShowName.Add(MainForm.weightSetConfig.carCaption);
  statShowName.Add(MainForm.weightSetConfig.faHuoCaption);
  statShowName.Add(MainForm.weightSetConfig.shouHuoCaption);
  statShowName.Add(MainForm.weightSetConfig.goodsCaption);
  statShowName.Add(MainForm.weightSetConfig.specCaption);
  statShowName.Add(MainForm.weightSetConfig.backup1Caption);
  statShowName.Add(MainForm.weightSetConfig.backup2Caption);
  statShowName.Add(MainForm.weightSetConfig.backup3Caption);
  statShowName.Add(MainForm.weightSetConfig.backup4Caption);
  statShowName.Add(MainForm.weightSetConfig.backup5Caption);
...
begin
  CBCount.Caption := '车次';
  CBGross.Caption := MainForm.weightSetConfig.grossCaption;
  CBTare.Caption := MainForm.weightSetConfig.tareCaption;
  CBNet.Caption := MainForm.weightSetConfig.netCaption;
  CBBundle.Caption := MainForm.weightSetConfig.bundleCaption;
  CBReal.Caption := MainForm.weightSetConfig.realCaption;
  CBSum.Caption := MainForm.weightSetConfig.sumCaption;
  CBQuanter.Caption := MainForm.weightSetConfig.quanterCaption;
  CBCost.Caption := MainForm.weightSetConfig.costCaption;
  CBBackup6.Caption := MainForm.weightSetConfig.backup6Caption;
  CBBackup7.Caption := MainForm.weightSetConfig.backup7Caption;
  CBBackup8.Caption := MainForm.weightSetConfig.backup8Caption;
  CBBackup9.Caption := MainForm.weightSetConfig.backup9Caption;
  CBBackup15.Caption := MainForm.weightSetConfig.backup15Caption;
  CBBackup16.Caption := MainForm.weightSetConfig.backup16Caption;
  CBBackup17.Caption := MainForm.weightSetConfig.backup17Caption;
  CBBackup18.Caption := MainForm.weightSetConfig.backup18Caption;
end;
...
procedure TDataQueryForm.NStat2Click(Sender: TObject);
begin
  if not TUserUtil.HasAuthority('打印报表') then
    Exit;

  TReportUtil.printStatReport(ExtractFilePath(ParamStr(0)) + '\StatReport\统计报表2.fr3');

end;

procedure TDataQueryForm.NStat3Click(Sender: TObject);
begin
  if not TUserUtil.HasAuthority('打印报表') then
    Exit;

  TReportUtil.printStatReport(ExtractFilePath(ParamStr(0)) + '\StatReport\统计报表3.fr3');

end;

procedure TDataQueryForm.NStat4Click(Sender: TObject);
begin
  if not TUserUtil.HasAuthority('打印报表') then
    Exit;

  TReportUtil.printStatReport(ExtractFilePath(ParamStr(0)) + '\StatReport\统计报表4.fr3');

end;

procedure TDataQueryForm.NStat5Click(Sender: TObject);
begin
  if not TUserUtil.HasAuthority('打印报表') then
    Exit;

  TReportUtil.printStatReport(ExtractFilePath(ParamStr(0)) + '\StatReport\统计报表5.fr3');

end;

procedure TDataQueryForm.NStat6Click(Sender: TObject);
begin
  if not TUserUtil.HasAuthority('打印报表') then
    Exit;

  TReportUtil.printStatReport(ExtractFilePath(ParamStr(0)) + '\StatReport\统计报表6.fr3');

end;

procedure TDataQueryForm.NStat7Click(Sender: TObject);
begin
  if not TUserUtil.HasAuthority('打印报表') then
    Exit;

  TReportUtil.printStatReport(ExtractFilePath(ParamStr(0)) + '\StatReport\统计报表7.fr3');
...
procedure TDataQueryForm.N11Click(Sender: TObject);
begin
  if not TUserUtil.HasAuthority('数据导出') then
    Exit;
