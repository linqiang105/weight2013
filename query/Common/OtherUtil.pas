...
uses SysUtils, DB, Classes, Variants, RzCmboBx, ADODB, SpeechLib_TLB, IdMessage;
...
type
  ...
  private
  public
    class function speak(s: string): Boolean;
  end;
...
uses QueryDM, Main, CommonUtil, DateUtils;
...
class function TVoiceUtil.speak(s: string): Boolean;
begin
  if (s = '') then
    Exit;
  MainForm.SpVoice1.Speak('', SVSFPurgeBeforeSpeak);
  MainForm.SpVoice1.Speak(s, SVSFlagsAsync);
end;
